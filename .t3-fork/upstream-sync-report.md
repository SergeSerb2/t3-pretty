...
- Preview AX snapshots stay node/byte-capped (`boundAccessibilityTree`, `t3TruncatedNodeCount`) and `Accessibility.getFullAXTree` is still depth-capped at 12. In-flight diagnostic request map evicts at `DIAGNOSTIC_REQUEST_LIMIT`. Agent cursor click lead stays 300ms so it exceeds the renderer 280ms glide. Desktop local-auth wait/retry tests stay with `waitForReady` on the pool mock.
