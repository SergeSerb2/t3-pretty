...
Check the account reported in provider settings after signing in. To move an
existing thread to another account, pick that Claude instance in the thread's model
picker and send your next message. T3 Code copies the Claude session into that
instance's config directory and resumes it there, so the conversation continues
under the new account. Other Claude sessions stay in their own directories.
