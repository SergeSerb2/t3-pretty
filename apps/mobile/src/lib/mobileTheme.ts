...
export const MOBILE_THEME_OPTIONS: ReadonlyArray<{
  ...
  { id: DEFAULT_MOBILE_THEME_ID, label: "T3 Pretty" },
  { id: "material-you", label: "Material You" },
  ...BUILT_IN_THEMES.map((theme) => ({
    id: theme.id as MobileThemeId,
    label: theme.id === BORING_MOBILE_THEME_ID ? "Boring" : theme.label,
  })),
];
...
export function normalizeMobileThemeId(value: unknown): MobileThemeId {
  return MOBILE_THEME_IDS.find((themeId) => themeId === value) ?? DEFAULT_MOBILE_THEME_ID;
}
...
export function getMobileThemeVariables(
  ...
  if (themeId === "material-you") {
    const theme = BUILT_IN_THEMES.find((candidate) => candidate.id === themeId) ?? T3_CHAT_THEME;
    colors = getThemeColorsForAppearance(theme, appearance) ?? theme.colors;
  } else {
    colors = getMobileThemeColors(themeId, appearance);
  }
  ...
  const mobileColors =
    themeId === DEFAULT_MOBILE_THEME_ID
      ...
      : colors;
  let baseVariables = createMobileThemeVariables(mobileColors, appearance, groupedCard);

  if (themeId === DEFAULT_MOBILE_THEME_ID) {
    const sharedBaseVariables = createMobileThemeVariables(colors, appearance);
    const prettyDefaultVariables = DEFAULT_MOBILE_THEME_VARIABLES[appearance];

    // Reapply Pretty's default palette only where the parent's native mobile
    // treatment leaves the corresponding token unchanged.
    baseVariables = Object.fromEntries(
      MOBILE_THEME_VARIABLE_NAMES.map((name) => [
        name,
        baseVariables[name] === sharedBaseVariables[name]
          ? prettyDefaultVariables[name]
          : baseVariables[name],
      ]),
