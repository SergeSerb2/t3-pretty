...
describe("buildThreadFeed", () => {
  ...
  it("keeps the working row when streaming text sits above a later tool group", () => {
    ...
    const presented = deriveThreadFeedPresentation(feed, null, new Set(), new Set(), startedAt);
    expect(presented.map((entry) => entry.type)).toEqual(["message", "work-toggle", "working"]);
  });
  ...
  it("models structured work-log overflow as command summary rows", () => {
    const activity = (
      id: string,
      createdAt: string,
      status: ThreadFeedActivity["status"] = "success",
  ...
  it("keeps image generation rows visible without a zero-count toggle", () => {
    const activity = (
      ...
      workEntry: {
        ...
        createdAt: createdAt,
        turnId: null,
        label: itemType === "image_generation" ? `Image ${id}` : `Tool ${id}`,
        tone: "tool",
      },
    });
    ...
    expect(imagesAndTail.map((entry) => entry.id)).toEqual([
      "image-1",
      "work-toggle:work-group:tool-1",
    ]);
  ...
  it("collapses a tool completion into its in-progress row across builder calls", () => {
    ...
    expect(group.activities).toHaveLength(1);
    expect(group.activities[0]?.id).toBe("tool-updated-incremental");
  });
  ...
  it("preserves presented row identity across derivations with unchanged inputs", () => {
    const activity = (id: string, createdAt: string): ThreadFeedActivity => ({
      ...
      toolLike: true,
      status: "success",
      workEntry: { id: id, createdAt: createdAt, turnId: null, label: `Tool ${id}`, tone: "tool" },
    });
    const feed: ThreadFeedEntry[] = [
      {
        type: "activity-group",
    ...
    // Expanded detail rows remain stable across unchanged derivations.
    const expanded = deriveThreadFeedPresentation(
      feed,
      null,
      new Set(),
    ...
    expect(expanded[1]).toMatchObject({
      activities:
        feed[0]?.type === "activity-group" ? feed[0].activities.map(({ id }) => ({ id })) : [],
    });
