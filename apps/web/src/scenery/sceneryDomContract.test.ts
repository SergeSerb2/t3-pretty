...
describe("glass contract with upstream chrome", () => {
  ...
  it("the top band is one surface and the column edge starts below it", () => {
    ...
    expect(indexCssSource).toContain("--workspace-frame-radius: 12px");
    expect(indexCssSource).toContain("circle at 100% 100%");
    expect(indexCssSource).not.toContain(
      "border-bottom-right-radius: var(--workspace-frame-radius)",
    );
    expect(indexCssSource).toContain(
    ...
    expect(sceneryCssSource).not.toMatch(
      /\[data-sidebar-frame-corner\][^{]*\{[^}]*background:\s*transparent/,
    );
