...
/**
 * Survives the draft-to-thread route swap while an async worktree setup is
 * still running. Entries are removed once the setup settles.
 */
...
export default function ChatView(props: ChatViewProps) {
  ...
  // thread that was set up when T3 Pretty still has an owner ref, and also
  // attaches to the route thread so a remount, reload, or second client can
  // pick up a recorded bootstrap the same way.
  ...
  const worktreeSetupQuery = useEnvironmentQuery(
    (worktreeSetupRef !== null && worktreeSetupActive) || routeThreadPreparesWorktree
      ? vcsEnvironment.worktreeSetup({
          environmentId: worktreeSetupRef?.environmentId ?? routeThreadRef.environmentId,
          input: { threadId: worktreeSetupRef?.threadId ?? routeThreadRef.threadId },
        })
      : null,
  );
  ...
  const liveWorktreeSetup =
    heldWorktreeSetup &&
    heldWorktreeSetup.threadId === (worktreeSetupRef?.threadId ?? routeThreadRef.threadId)
      ? heldWorktreeSetup
      : null;
  ...
  // async setup script can keep the snapshot running past the handoff. A
  // successful row exits once the script settles; a failed script stays for
  // the rest of the turn so the exit code and terminal remain reachable.
