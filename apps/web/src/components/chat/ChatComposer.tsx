...
const sharedRuntimeModeConfig = runtimeModeConfig as Record<
  RuntimeMode,
  {
    label: string;
    description: string;
    icon: (typeof runtimeModeConfig)["auto"]["icon"];
  }
>;
const hasSharedYoloRuntimeMode = "yolo" in runtimeModeConfig;

if (!hasSharedYoloRuntimeMode) {
  sharedRuntimeModeConfig.yolo = {
    label: "Yolo",
    description: "Full access. Can stop to ask questions.",
    icon: sharedRuntimeModeConfig.auto.icon,
  };

  const sharedRuntimeModeOptions = runtimeModeOptions as RuntimeMode[];
  if (!sharedRuntimeModeOptions.includes("yolo")) {
    const fullAccessIndex = sharedRuntimeModeOptions.indexOf("full-access");
    sharedRuntimeModeOptions.splice(
      fullAccessIndex === -1 ? sharedRuntimeModeOptions.length : fullAccessIndex,
      0,
      "yolo",
    );
  }
...
const ComposerFooterModeControls = memo(function ComposerFooterModeControls(props: {
  ...
  const runtimeModeOption = sharedRuntimeModeConfig[props.runtimeMode];
  ...
  return (
    <>
      ...
      <ComposerPrimaryActions
        ...
        {...(props.onQueueSend ? { onQueueSend: props.onQueueSend } : {})}
...
export interface ChatComposerProps {
  ...
  onSend: (
    ...
    options?: { delivery?: TurnDeliveryMode },
  ) => void;
  onInterrupt: () => void;
  onImplementPlanInNewThread: () => void;
  onRespondToApproval: (
...
export const ChatComposer = memo(function ChatComposer(props: ChatComposerProps) {
    ...
    (
      ...
      const submission = submitComposerDraft({
        ...
        onSend: (sendEvent) => {
          ...
          onSend(sendEvent, intent, delivery ? { delivery } : undefined);
  ...
  const onComposerCommandKey = (
    ...
    const submissionIntent =
      key === "Enter"
        ...
        : null;
    if (submissionIntent) {
      // Alt+Enter queues instead of steering a running or starting turn.
      submitComposer(
        undefined,
        submissionIntent,
        event.altKey && composerTurnIsInProgress(phase) ? "queue" : undefined,
      );
      return true;
    }
  ...
  const handleQueueSendPrimaryAction = useCallback(() => {
    submitComposer(undefined, "foreground", "queue");
  }, [submitComposer]);
                          ...
                          >
                            ...
                            <Button
                              ...
                              className={cn(
                                ...
                                image.source?.kind === "snap-shot" &&
                                  "opacity-0 transition-opacity pointer-coarse:opacity-100 tesla-touch:opacity-100 focus-visible:opacity-100 group-hover/attachment:opacity-100 group-focus-within/attachment:opacity-100",
                              )}
              ...
              >
                ...
                >
                  ...
                  <ComposerFooterPrimaryActions
                    ...
                    pendingAction={pendingPrimaryAction}
                    isRunning={composerTurnIsInProgress(phase)}
                    showPlanFollowUpPrompt={
                    ...
                    {...(composerTurnIsInProgress(phase)
                      ? { onQueueSend: handleQueueSendPrimaryAction }
                      : {})}
                    onPreviousPendingQuestion={onPreviousActivePendingUserInputQuestion}
                    onInterrupt={handleInterruptPrimaryAction}
                    onImplementPlanInNewThread={handleImplementPlanInNewThreadPrimaryAction}
