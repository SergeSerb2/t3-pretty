  ...
  return (
    <>
      ...
      <SettingsRow
        ...
        description={`Send activity to mobile notifications and Live Activities without ${SURGE_CONNECT_NAME}.`}
        control={
          <CloudLinkSwitch
    ...
    () =>
      environments
        .filter((environment) => {
          ...
          if (target._tag === "PrimaryConnectionTarget") return false;
          // Direct LAN/Tailscale connections are no longer a supported way to
          // reach a remote machine — Surge Connect (or SSH on desktop) is.
          // Desktop-local backends (WSL) are host-managed and stay.
          if (target._tag === "BearerConnectionTarget" && !isDesktopLocalConnectionTarget(target)) {
            return false;
          }
          return true;
        })
  ...
  const visibleSavedEnvironments = useMemo(() => {
    const visibleIds = selectVisibleRemoteEnvironmentIds(
      savedEnvironments.map((environment) => ({
        ...
        // Desktop-local backends (WSL) never collapse into a machine group.
        machineKey: isDesktopLocalConnectionTarget(environment.entry.target)
          ? `local:${environment.environmentId}`
          : environmentMachineKey(environment.label),
        priority: connectionPhaseGroupPriority(environment.connection.phase),
        working: isWorkingConnectionPhase(environment.connection.phase),
      })),
    );
        ...
        savedEnvironments
          .filter(
            (environment) =>
              ...
              isWorkingConnectionPhase(environment.connection.phase),
          )
          .map((environment) => environmentMachineKey(environment.label)),
      ),
    [savedEnvironments],
  );
    ...
    return (
      <>
        ...
        {desktopWslState.enabled ? (
          <SettingsRow
            title="WSL only"
            description="Run only the WSL backend without a second Windows backend process. Useful if you develop entirely inside WSL. T3 Pretty restarts when this changes."
          ...
          >
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  {pendingDesktopServerExposureMode === "network-accessible"
                    ? "Let your other devices connect to T3 Pretty over the network. Pair devices to give them access. T3 Pretty will restart."
                    : "Devices connected over your local network will disconnect. Existing tunnels, such as T3 Connect or Tailscale HTTPS, keep working. T3 Pretty will restart."}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
            ...
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  {pendingWslChange?.kind === "disable"
                    ? pendingWslChange.wasWslOnly
                      ? "T3 Pretty will restart on the Windows backend. Threads and projects opened against WSL stay safe inside the distro and become available again when you re-enable WSL."
                      : "The WSL backend will stop. Threads and projects opened against WSL stay safe inside the distro, but they'll be unavailable in T3 Pretty until you re-enable WSL."
                    : pendingWslChange?.kind === "distro"
                      ? "T3 Pretty will restart the WSL backend on the new distro. Sessions still running on the current distro will be interrupted."
                      : pendingWslChange?.kind === "enable"
                        ? "Run the WSL backend alongside the Windows one, or stop the Windows backend and use only WSL? You can change this later from Settings."
                        : pendingWslChange?.nextValue
                          ? "T3 Pretty will restart and start only the WSL backend. Your Windows-side projects won't be accessible until you turn this off again."
                          : "T3 Pretty will restart and bring the Windows backend back up alongside WSL."}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
          ...
          >
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  T3 Pretty will restart the local backend without Tailscale Serve.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
          ...
          >
            <DialogPopup className="max-w-md">
              <DialogHeader>
                ...
                <DialogDescription>
                  T3 Pretty will restart the local backend with Tailscale Serve enabled and ask
  ...
  return (
    <SettingsPageContainer width="wide">
      ...
      <SettingsSection
        ...
        title="Environments"
        headerAction={
          savedServerUpdateTargets.length > 0 || desktopBridge ? (
            <div className="flex items-center gap-1">
              ...
              {/* Desktop is the only surface that can add an environment by hand
                  (SSH); everything else connects through Surge Connect below. */}
              {desktopBridge ? (
                <Dialog
            ...
            >
              <div className="divide-y divide-border/50" aria-label="Environments">
                {[
                  ...
                  ...(showHiddenSavedEnvironments ? hiddenSavedEnvironments : []),
                ].map((environment) => {
                    ...
                    >
                      ...
                      <span className="min-w-0 flex-1 space-y-1">
                        ...
                        <Tooltip>
                          ...
                          >
                            {environment.displayUrl ?? "Surge Connect"}
                          </TooltipTrigger>
                          <TooltipPopup>{environment.displayUrl ?? "Surge Connect"}</TooltipPopup>
                        </Tooltip>
