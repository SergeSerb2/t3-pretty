          ...
          <WorkspacePageContainer width="wide">
            {selectedEnvironments.length === 0 ? (
              ...
              <>
                ...
                />
                {sourceMessages
                  .filter((message) => !merged.sourceWarnings.includes(message))
                  .map((message) => (
...
/**
 * Explains incomplete totals from failed or incompatible environments and
 * deduplicated transcripts. Environments that are still answering never reach
 * this notice; the page shows the loading skeleton until every connected
 * environment is terminal. Shown inside the environment filter so arriving
...
function UsageEnvironmentFilter({
  ...
  useStatusPulse(showUsageStatus && pendingCount > 0);
        ...
        >
          ...
          <span className="flex size-3.5 shrink-0 items-center justify-center text-muted-foreground">
            {showUsageStatus && pendingCount > 0 ? (
              <>
                <CircleDashedIcon className="status-pulse size-3.5" aria-hidden />
