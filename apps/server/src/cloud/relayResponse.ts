...
export const filterRelayResponse = Effect.fn("cloud.filter_relay_response")(function* (
  ...
  const decoded = yield* Effect.gen(function* () {
    if (Number(response.headers["content-length"]) > maxBytes) {
      ...
      return Option.none<RelayProtectedError>();
    }
    ...
    if (body.truncated || body.invalidUtf8) return Option.none<RelayProtectedError>();
    return yield* Schema.decodeUnknownEffect(Schema.fromJsonString(RelayProtectedError))(
      body.text,
    ).pipe(Effect.option);
  }).pipe(Effect.orElseSucceed(() => Option.none<RelayProtectedError>()));
