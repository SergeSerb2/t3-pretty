...
const systemdUnit = BootService.BOOT_SERVICE_UNIT_FILE;
const launchdLabel = BootService.BOOT_SERVICE_LAUNCHD_LABEL;
const launchdPlist = BootService.BOOT_SERVICE_PLIST_FILE;
...
it("keeps systemd pinned to the stable launcher rather than a versioned server", () => {
  const unit = BootService.renderBootServiceUnit({
    ...
    unitPath: "/home/theo/.config/systemd/user/t3pretty.service",
  });
...
const macPlan = {
  ...
  unitPath: "/Users/theo/Library/LaunchAgents/com.sergeserb.t3pretty.service.plist",
};
  ...
  const runner = ProcessRunner.ProcessRunner.of({
    run: Effect.fn("test.run_boot_service_command")(function* (
      ...
      if (!failed && command === "loginctl enable-linger --no-ask-password 501")
        control.linger = "yes";
      if (!failed && command === "systemctl --user enable t3pretty.service") control.enabled = true;
      if (!failed && command === "systemctl --user restart t3pretty.service") control.active = true;
      if (
        control.stateAfterStop !== undefined &&
        (command === "systemctl --user stop t3pretty.service" ||
...
it.layer(NodeServices.layer)("boot service install", (it) => {
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(commands).not.toContain("systemctl --user stop t3pretty.service");
      }),
  );
  ...
  it.effect("installs, reports current state, and uninstalls", () =>
    Effect.gen(function* () {
      ...
      expect(timeouts.get(`systemctl --user disable --now ${systemdUnit}`)).toEqual(
    ...
    (platform) =>
      Effect.gen(function* () {
        ...
        expect(
          ...
          platform === "linux"
            ? [
                "systemctl --user stop t3pretty.service",
                "systemctl --user restart t3pretty.service",
              ]
            : [
                "launchctl bootout --wait gui/501/com.sergeserb.t3pretty.service",
  ...
  it.effect("installs, reports current state, and uninstalls on macOS", () =>
    Effect.gen(function* () {
      ...
      expect(plan.unitPath.endsWith(path.join("Library", "LaunchAgents", launchdPlist))).toBe(true);
      ...
      expect(timeouts.get(`launchctl bootout --wait gui/501/${launchdLabel}`)).toEqual(
        Duration.seconds(120),
      );
    }),
  );
  ...
  it.effect("restarts the launch agent when repair fails", () =>
    Effect.gen(function* () {
      ...
      expect(commands.filter((command) => command.startsWith("launchctl "))).toEqual([
        `launchctl bootout --wait gui/501/${launchdLabel}`,
        `launchctl enable gui/501/${launchdLabel}`,
  ...
  it.effect("ignores a bootout for an agent that is not loaded", () =>
    Effect.gen(function* () {
      ...
      yield* service.install();
      control.failCommand = `launchctl bootout --wait gui/501/${launchdLabel}`;
