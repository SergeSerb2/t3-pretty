 ...
 * `createRequire` (SEA cannot `import` a file-backed package), so the
 * installed copy needs a `require` condition before the CLI can start.
 */
...
const installFromForkCliTarball = Effect.fn("cloud.pinned_runtime.install_cli_tarball")(function* (
  ...
  if (yield* fs.exists(fffNodePackageJson).pipe(Effect.orElseSucceed(() => false))) {
    const raw = yield* fs
      .readFileString(fffNodePackageJson)
      .pipe(
        Effect.mapError(
          (cause) =>
