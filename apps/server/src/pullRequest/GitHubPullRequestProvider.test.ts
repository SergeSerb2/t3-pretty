...
describe("gitHubViewerPermissions", () => {
  ...
  it.effect("puts GraphQL reactor counts on the conversation, which include Codex", () =>
    Effect.gen(function* () {
      const provider = yield* make;
      ...
      expect(activity.reactions).toEqual([
        { content: "thumbs-up", count: 1, actors: [], viewerHasReacted: false },
      ]);
    }).pipe(
      Effect.provide(
        Layer.mock(GitHubPullRequestCli.GitHubPullRequestCli)({
          ...
          listReviewThreadComments: () =>
            Effect.succeed({
              ...
              truncated: false,
              reactions: [
                { content: "thumbs-up" as const, count: 1, actors: [], viewerHasReacted: false },
              ],
