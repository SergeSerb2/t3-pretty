 ...
 * overwritten — a user-owned folder that already contains a readable
 * `SKILL.md` wins the name, and no root is created while there is nothing
 * to write.
...
const make = Effect.gen(function* () {
  ...
  const materialize = Effect.fn("SkillMaterializer.materialize")(function* (input: {
    ...
    for (const skillId of input.skillIds) {
      ...
      const parsedHost = "dir" in skill ? parseHostSkillId(skillId) : null;
      resolved.push({
        id: skillId,
        name: skill.name,
        sourceRepo:
          "sourceRepo" in skill ? skill.sourceRepo : `host/${parsedHost?.originKey ?? "unknown"}`,
        sourcePath: "sourcePath" in skill ? skill.sourcePath : (parsedHost?.dirName ?? skill.name),
        storeDir,
      });
    }
