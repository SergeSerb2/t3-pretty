...
describe("ThreadLiveEventCoalescer", () => {
  ...
  it.effect("flushes persisted updates before preserving an ephemeral event envelope", () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        yield* coalescer.offer(ephemeral);
        const delivered = yield* coalescer.stream.pipe(Stream.take(2), Stream.runCollect);
        expect(delivered[0]).toMatchObject({ kind: "event", event: { sequence: 2 } });
        expect(delivered[1]).toEqual(ephemeral);
      }),
    ),
  );
