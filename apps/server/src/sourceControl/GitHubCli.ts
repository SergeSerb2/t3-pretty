...
export class GitHubCodexReviewDecodeError extends Schema.TaggedError<GitHubCodexReviewDecodeError>()(
...
export const make = Effect.gen(function* () {
  ...
  const apiQuota = createGitHubApiQuota();
  ...
  const executeRaw: GitHubCli["Service"]["execute"] = Effect.fn("GitHubCli.executeRaw")(
    function* (input) {
      ...
      const host = credential === null ? gitHubApiHostFromArgs(input.args) : credential.host;
      const now = yield* Clock.currentTimeMillis;
      if (apiQuota.blockedUntil(host, now) !== null) {
      ...
      return yield* process
        ...
        .pipe(
          ...
          Effect.tap(() => Effect.sync(() => apiQuota.noteSuccess(host))),
          Effect.tapError((error) =>
            Effect.sync(() => {
              if (error._tag === "GitHubCliRateLimitError") {
                apiQuota.noteRateLimit(host, now);
              }
            }),
          ),
        );
