...
function formatCloudStatus(status: CloudCliStatus, options?: { readonly json?: boolean }): string {
  ...
  const nextStep = !status.authenticated
    ? `Run \`t3 connect link\` to authorize and enable ${SURGE_CONNECT_NAME}.`
    : !status.desired
      ? `Run \`t3 connect link\` to enable ${SURGE_CONNECT_NAME}.`
      : !status.linked
...
const confirmRelayClientInstall = (version: string) =>
  Prompt.run(
    Prompt.confirm({
      message: `The T3 relay client is required for ${SURGE_CONNECT_NAME}. Download and install version ${version}?`,
      initial: false,
    }),
  );
...
export const reportCloudDisconnectResults = Effect.fn("cloud.cli.report_disconnect_results")(
  function* (input: {
    ...
    if (input.liveResult.status === "failed") {
      ...
      yield* Console.log(`${SURGE_CONNECT_NAME} is disabled locally.`);
    }
...
const linkEnvironmentForConnect = Effect.fn("cloud.cli.link_environment")(function* (options: {
  ...
  if (!publishOnly) {
    ...
    if (Option.isNone(installed)) {
      yield* Console.log(
        `${SURGE_CONNECT_NAME} setup cancelled. The relay client was not installed.`,
      );
      return null;
    }
...
const connectLoginCommand = Command.make("login", {
  ...
  Command.withDescription(
    `Authorize the ${SURGE_CONNECT_NAME} CLI without enabling remote access.`,
  ),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
        yield* Console.log(`${SURGE_CONNECT_NAME}\n`);
...
const connectLinkCommand = Command.make("link", {
  ...
  Command.withDescription(`Authorize this environment for ${SURGE_CONNECT_NAME} on next start.`),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
...
const connectStatusCommand = Command.make("status", {
  ...
  Command.withDescription(`Show persisted ${SURGE_CONNECT_NAME} and relay client state.`),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
      ...
      Effect.gen(function* () {
        ...
        if (!enabled) {
          ...
          if (!linkedNow && (yield* CliState.readCliDesiredLinkMode) === "publish_only") {
            yield* CliState.setCliDesiredCloudLink(false);
            yield* Console.log(`Cancelled the pending publish-only ${SURGE_CONNECT_NAME} link.`);
          }
        ...
        if (yield* CliState.readCliDesiredCloudLink) {
          yield* Console.log(
            `A ${SURGE_CONNECT_NAME} link is already pending. Start T3 to finish provisioning it; publishing starts once it links.`,
          );
          return;
        }
...
const connectUnlinkCommand = Command.make("unlink", {
  ...
  Command.withDescription(
    `Disable ${SURGE_CONNECT_NAME} while retaining the stored authorization.`,
  ),
  Command.withHandler((flags) =>
...
const connectLogoutCommand = Command.make("logout", {
  ...
  Command.withDescription(`Disable ${SURGE_CONNECT_NAME} and clear the stored CLI authorization.`),
  Command.withHandler((flags) =>
...
export const connectCommand = Command.make("connect", {
  ...
  Command.withDescription(`Set up ${SURGE_CONNECT_NAME} for this machine.`),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
