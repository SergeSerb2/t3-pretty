...
it("gives a direct repair command for a stale service", () => {
  assert.include(
    ...
    `Next: Run \`${forkCliCommand("service install")}\` to repair it.`,
  );
});
...
it("explains an incomplete nightly installation and keeps repair on its installed version", () => {
  ...
  expect(output).toContain(forkCliCommand("service install"));
...
it("reports a newer installed service and tells the CLI to catch up to it", () => {
  ...
  assert.include(output, forkCliCommand("update", "0.0.32-nightly.1"));
  assert.notInclude(output, "npx t3@latest");
});
