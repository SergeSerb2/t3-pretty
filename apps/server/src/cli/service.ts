...
export function formatServiceStatus(
  ...
  if (
    ...
    return [
      ...
      `  Logs: ${status.logPath}`,
      ...problems,
      `  Next: Run \`${forkCliCommand("update", installedVersion)}\` to match it, or pass \`--allow-downgrade\` to \`${forkCliCommand("service install")}\` explicitly.`,
    ].join("\n");
  }
  return [
    "T3 Code service",
    ...
    `  Logs: ${status.logPath}`,
    ...problems,
    ...(status.current ? [] : [`  Next: Run \`${forkCliCommand("service install")}\` to repair it.`]),
  ].join("\n");
}
...
const serviceUpdateCommand = Command.make("update", serviceReconcileFlags).pipe(
  Command.withDescription(
    `Deprecated. Run \`${forkCliCommand("update")}\` to move to a newer release.`,
  ),
  Command.unlisted,
  Command.withHandler((flags) =>
    runServiceCommand(
