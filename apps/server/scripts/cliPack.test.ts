// @effect-diagnostics nodeBuiltinImport:off preferSchemaOverJson:off globalErrorInEffectCatch:off globalErrorInEffectFailure:off - Fixture packs a real tarball and inspects it with tar.
...
it.layer(NodeServices.layer)("packServerCli", (it) => {
  it.effect("packs resolved catalog deps and restores the working tree", () =>
    Effect.gen(function* () {
      ...
      assert.equal(
        yield* fs.readFileString(path.join(serverDir, "package.json")),
        originalPackageJson,
      );
