// This file mostly exists because we want dev mode to say "T3 Pretty (Dev)" instead of "electron"
...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
export const APP_DISPLAY_NAME = isDevelopment ? "T3 Pretty (Dev)" : "T3 Pretty (Alpha)";
export const APP_BUNDLE_ID = isDevelopment
  ? `com.t3tools.t3code.dev.${devBundleIdSuffix || "local"}`
  : "com.t3tools.t3code";
const APP_PROTOCOL_SCHEMES = isDevelopment ? ["t3code-dev"] : ["t3code"];
const LAUNCHER_VERSION = 15;
const developmentMacIconPngPath = NodePath.join(repoRoot, "assets", "pretty", "t3-pretty-1024.png");
const productionMacIconPngPath = NodePath.join(repoRoot, "assets", "pretty", "t3-pretty-1024.png");
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the T3 Pretty name.
