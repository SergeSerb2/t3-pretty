...
describe("electron development launcher", () => {
  ...
  it("keeps the native Electron executable name inside the branded macOS bundle", () => {
    const paths = resolveMacLauncherPaths(
      "/repo/apps/desktop/.electron-runtime/T3 Pretty (Dev).app",
      "T3 Pretty (Dev)",
    );

    assert.equal(paths.launcherExecutableName, "T3 Pretty (Dev) Launcher");
    assert.equal(
      paths.launcherBinaryPath,
      "/repo/apps/desktop/.electron-runtime/T3 Pretty (Dev).app/Contents/MacOS/T3 Pretty (Dev) Launcher",
    );
    assert.equal(
      paths.runtimeElectronBinaryPath,
      "/repo/apps/desktop/.electron-runtime/T3 Pretty (Dev).app/Contents/MacOS/Electron",
    );
    ...
    assert.include(
      script,
      "exec '/repo/apps/desktop/.electron-runtime/T3 Pretty (Dev).app/Contents/MacOS/Electron'",
    );
  ...
  it("derives launcher icons from canonical development and production assets", () => {
    ...
    assert.match(development.sourceIconPath, /assets\/pretty\/t3-pretty-1024\.png$/);
    assert.equal(development.generatedIconPath, "/runtime/icon-dev.icns");
    assert.match(production.sourceIconPath, /assets\/pretty\/t3-pretty-1024\.png$/);
