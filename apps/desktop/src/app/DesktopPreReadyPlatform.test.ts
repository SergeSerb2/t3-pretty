  ...
  for (const previousEntry of [undefined, 'Exec="/Applications/deleted-previous.AppImage" %U']) {
    it.effect(
      ...
      () => {
        ...
        getSwitchValueMock.mockReturnValue("");
        let desktopName = "t3pretty.desktop";
        ...
        writeFileSyncMock.mockImplementation((path: string, contents: string) => {
          if (path === "/xdg/applications/com.t3tools.T3Pretty.desktop") desktopEntry = contents;
        });
        ...
        return Effect.scoped(
          Effect.gen(function* () {
            ...
            assert.equal(identity.desktopName, "com.t3tools.T3Pretty.desktop");
            assert.include(identity.desktopEntry ?? "", 'Exec="/Applications/current.AppImage" %U');
            assert.include(identity.desktopEntry ?? "", "Name=T3 Pretty");
