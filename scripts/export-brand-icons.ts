...
export const exportBrandIcons = Effect.fn("exportBrandIcons")(function* (checkOnly: boolean) {
  ...
  // The assets/pretty family is fork-owned art regenerated by
  // scripts/generate-pretty-icons.py. Icon Composer renders must not overwrite
  // those paths or be checked against them.
  const forkOwnedPrefix = "assets/pretty/";
  for (const variant of ICON_VARIANTS) {
    ...
    for (const [relativePath, contents] of variantAssets) {
      if (relativePath.startsWith(forkOwnedPrefix)) continue;
