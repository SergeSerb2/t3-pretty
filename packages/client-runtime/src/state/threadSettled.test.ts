...
describe("changeRequestAutoSettles", () => {
  it.each([
    ...
    expect(changeRequestAutoSettles(state === null ? null : { state }, {})).toBe(expected);
  });
  ...
  it("ignores a closed change request last touched before the thread existed", () => {
    expect(
      changeRequestAutoSettles(
        { state: "closed", updatedAt: "2026-03-31T23:59:59.999Z" },
        { thread: idleThread },
      ),
    ).toBe(false);
  });

  it("settles on a closed change request touched at or after the thread's latest event", () => {
    for (const updatedAt of [THREAD_CREATED_AT, "2026-04-02T00:00:00.000Z"]) {
      expect(changeRequestAutoSettles({ state: "closed", updatedAt }, { thread: idleThread })).toBe(
        true,
      );
    }
  });

  it("never re-settles a thread revived after the change request closed", () => {
    // Settling on a close happens once: a user message newer than the PR's
    ...
    expect(
      changeRequestAutoSettles(
        { state: "closed", updatedAt: "2026-04-03T00:00:00.000Z" },
        { thread: revived },
      ),
    ).toBe(false);
    // A close landing after the revival still settles.
    expect(
      changeRequestAutoSettles(
        { state: "closed", updatedAt: "2026-04-06T00:00:00.000Z" },
        { thread: revived },
      ),
    ).toBe(true);
  });

  it("still settles when the change request closes during an in-flight turn", () => {
    // Anchor is user-initiated activity only: the agent finishing a turn
    // after the close must not block the settle the close earned.
    const midTurnClose = {
      createdAt: THREAD_CREATED_AT,
    ...
    expect(
      changeRequestAutoSettles(
        { state: "closed", updatedAt: "2026-04-02T00:10:00.000Z" },
        { thread: midTurnClose },
      ),
    ).toBe(true);
  });
  ...
  it("falls back to settling when either timestamp is missing or malformed", () => {
    expect(changeRequestAutoSettles({ state: "closed" }, { thread: idleThread })).toBe(true);
    expect(
      changeRequestAutoSettles({ state: "closed", updatedAt: null }, { thread: idleThread }),
    ).toBe(true);
    expect(
      changeRequestAutoSettles({ state: "closed", updatedAt: "2026-03-01T00:00:00.000Z" }, {}),
    ).toBe(true);
    expect(
      changeRequestAutoSettles(
...
describe("effectiveSettled", () => {
  ...
  it("ignores a change request that closed before the thread's latest event", () => {
    // A new thread started at a worktree root can inherit the branch's old
    // closed PR, and a revived thread can outlive its closure; neither
    // should settle the live conversation.
    const fresh = makeShell({ activityAt: FRESH });
    expect(
      effectiveSettled(fresh, {
        now: NOW,
        autoSettleAfterDays: null,
        changeRequest: { state: "closed", updatedAt: "2026-03-20T00:00:00.000Z" },
      }),
    ).toBe(false);
    // A close during the thread's life still settles it.
    expect(
      effectiveSettled(fresh, {
        now: NOW,
        autoSettleAfterDays: null,
        changeRequest: { state: "closed", updatedAt: "2026-04-09T00:00:00.000Z" },
      }),
    ).toBe(true);
  });
