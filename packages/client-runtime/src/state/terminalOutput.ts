...
function appendOutput(
  ...
  const trailing = last?.data.charCodeAt(last.data.length - 1);
  const leading = data.charCodeAt(0);
  if (
    last &&
    trailing !== undefined &&
    trailing >= 0xd800 &&
    ...
    const prefix = last.data.slice(0, -1);
    return appendOutput(
      {
        ...current,
        chunks:
          prefix.length === 0
            ? current.chunks.slice(0, -1)
            : [
                ...current.chunks.slice(0, -1),
                { ...last, data: prefix, byteLength: last.byteLength - 3 },
              ],
        retainedBytes: current.retainedBytes - 3,
        nextOffset: current.nextOffset - 1,
      },
      last.data.slice(-1) + data,
      maxBufferBytes,
    );
  }
  if (maxBufferBytes <= 0) {
