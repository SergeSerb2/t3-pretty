...
describe("terminal session reducers", () => {
  ...
  it("counts a surrogate pair split across output chunks as one code point", () => {
    ...
    expect(terminalOutputText(pair.output)).toBe("🙂");
    expect(pair.output.retainedBytes).toBe(4);
  });
  ...
  it("keeps a long output stream byte-trimmed across chunk appends", () => {
    ...
    expect(terminalOutputText(state.output)).toBe(`cdef${"abcdef".repeat(10)}`);
    expect(state.output.retainedBytes).toBe(64);
  ...
  it("exposes only new output after the retained buffer rolls over", () => {
    ...
    expect(terminalOutputText(current.output)).toBe("cdefghij");
    expect(
      readTerminalOutputUpdate(
        current.output,
        readTerminalOutputUpdate(previous.output, INITIAL_TERMINAL_OUTPUT_CURSOR).cursor,
      ),
    ).toMatchObject({ type: "append", data: "ij" });
  });
  ...
  it("drops a multi-byte character straddling the trim boundary whole", () => {
    ...
    expect(terminalOutputText(state.output)).toBe("abcdefgh");
    expect(state.output.retainedBytes).toBe(8);

    state = append(state, "🙂");
    expect(terminalOutputText(state.output)).toBe("efgh🙂");

    state = append(state, "ij");
    expect(terminalOutputText(state.output)).toBe("gh🙂ij");

    // The trim boundary lands inside the first emoji; it is dropped whole
    // rather than decoded as a partial sequence.
    state = append(state, "🙂");
    expect(terminalOutputText(state.output)).toBe("ij🙂");
    expect(state.output.retainedBytes).toBe(6);
  });
});
